...
export default function viteNodeCGPlugin(pluginConfig) {
    ...
    const srcDir = (_b = pluginConfig === null || pluginConfig === void 0 ? void 0 : pluginConfig.srcDir) !== null && _b !== void 0 ? _b : './src';
    const inputPatterns = [
    ...
    Object.keys(inputConfig).forEach((matchPath) => {
        if (!inputs.some((input) => minimatch(input, path.posix.join(srcDir, matchPath))))
    ...
    function injectAssetsTags(html, entry) {
        ...
        if (config.mode === 'development') {
            tags.push(`<script type="module" src="${dSrvProtocol}://${path.posix.join(dSrvHost, 'bundles', bundleName, '@vite/client')}"></script>`);
            tags.push(`<script type="module" src="${dSrvProtocol}://${path.posix.join(dSrvHost, 'bundles', bundleName, entry)}"></script>`);
        }
    ...
    function generateHTMLFiles() {
        ...
        for (const [filePath, htmlDoc] of Object.entries(htmlDocs)) {
            ...
            try {
                ...
                console.error(`vite-plugin-nodecg: Could not create directory ${dir} for input ${filePath}. Skipping...`);
                continue;
            }
    ...
    return {
        ...
        writeBundle() {
            ...
            try {
                ...
                assetManifest = JSON.parse(fs
                    .readFileSync(path.posix.join(process.cwd(), config.build.outDir, 'manifest.json'))
                    .toString());
            }
            catch (err) {
        ...
        configureServer(server) {
            server.httpServer.on('listening', () => {
                ...
                dSrvHost = `${(_a = server.config.server.host) !== null && _a !== void 0 ? _a : 'localhost'}:${(_b = server.config.server.port) !== null && _b !== void 0 ? _b : '5173'}`;
